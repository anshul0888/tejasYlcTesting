...
angular.module('com.ylc.orders')
    .controller('TravelersOrderCtrl', function($scope, CoreService,gettextCatalog, Dashboard,auth,User,$state,Gigs,SaveSearch,userData,Register,notify,orderDetails,$stateParams) {
        ...
        if(orderDetails.travelerId === auth.uid){
        ...
        Dashboard.tripsTaken(auth.uid).once('value', function (snapshot) {
            if(snapshot.val() !== null){
                ...
                $scope.localsData = User(orderDetails.localId);
            }  else {
